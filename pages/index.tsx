...
const Home: NextPage = ({ allLocalPostsData }: any) => {
  const { data } = usePosts(2);
  const recentPosts = allLocalPostsData.slice(0, 3);
  return (
    <>
      <Header
        title="Home"
        description="Welcome to Dapper Designer, James Aspinall's personal website & mini blog"
      />
      ...
      <div className="flex flex-col md:flex-row lg:flex-row xl:flex-row h-full w-full py-8">
        ...
        <div className="flex w-full h-full p-4 text-gray-800">
          <div className="w-full">
            ...
            <hr />
            {recentPosts.map(({ id, title }: IPostProps) => (
              <div className="border-b mb-4 pb-2 mt-4" key={id}>
                <p className="text-lg mb-1">{title}</p>

                <Link href={`/blogs/${id}`} passHref>
                  <p className="cursor-pointer hover:text-indigo-500 hover:font-semibold transition-all">
                    Read More <span className="text-indigo-500">&#10140;</span>
                  </p>
                </Link>
              </div>
            ))}
